        ...
        ZStack(){
            ColorPallete().mediumFGColor
                VStack(){
                    ...
                    let progressColor = bucket.value >= bucket.capacity ? Color.red : ColorPallete().accentColor
                    ZStack(){
                        VStack(spacing: -10){
                        ...
                        CircularProgressView(percentage: bucket.value / bucket.capacity, bgcolor: progressColor, fillColor:ColorPallete().mediumBGColor, strokeWidth: 20.0)
                            .frame(width: 150.0, height: 150.0)
                            .padding(0)
                    }
                    let spent = bucket.value
